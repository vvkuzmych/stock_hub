...
func (s *StockOrderService) GetOrderByID(orderID int64) (*model.StockOrder, error) {
	query := `SELECT id, user_id, username, symbol, order_type, price, quantity, status, created_at 
	          FROM stock_orders 
	          WHERE id = ?`
	query = sqlutil.ConvertPlaceholders(query, s.driver)
	...
	err := s.db.QueryRow(query, orderID).Scan(
		&order.ID, &order.UserID, &order.Username, &order.Symbol,
		&orderTypeStr, &order.Price, &order.Quantity, &order.Status, &order.CreatedAt,
	)
	if err != nil {
...
func (s *StockOrderService) GetOrdersByUserID(userID int64, limit int) ([]*model.StockOrder, error) {
	if limit <= 0 {
		limit = 100
	}
