.PHONY: help run run-postgres migrate-up migrate-down docker-up docker-down clean
...
run: ## Запустити з SQLite
	@echo "🚀 Starting with SQLite..."
	go run cmd/server/main.go

run-postgres: ## Запустити з PostgreSQL
	@echo "🐘 Starting with PostgreSQL..."
	DB_TYPE=postgres go run cmd/server/main.go

migrate-up: ## Застосувати міграції (SQLite)
	@echo "📊 Running migrations (SQLite)..."
	go run cmd/migrate/main.go -command=up

migrate-up-postgres: ## Застосувати міграції (PostgreSQL)
	@echo "🐘 Running migrations (PostgreSQL)..."
	DB_TYPE=postgres go run cmd/migrate/main.go -command=up

migrate-down: ## Відкотити останню міграцію (SQLite)
	@echo "↩️  Rolling back last migration (SQLite)..."
	go run cmd/migrate/main.go -command=down

migrate-down-postgres: ## Відкотити останню міграцію (PostgreSQL)
	@echo "↩️  Rolling back last migration (PostgreSQL)..."
...
migrate-down-all: ## Відкотити ВСІ міграції
	...
	@go run cmd/migrate/main.go -command=down 2>/dev/null || true
	@go run cmd/migrate/main.go -command=down 2>/dev/null || true
	@go run cmd/migrate/main.go -command=down 2>/dev/null || true
...
setup-postgres: docker-up ## Повне налаштування PostgreSQL
	...
	@echo "✅ Setup complete! Run: make run-postgres"
